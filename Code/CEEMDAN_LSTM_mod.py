...
from keras.layers import Dense, Activation, Conv1D, LSTM, Dropout, Reshape, Bidirectional,Flatten
...
def EEMD_LSTM_Model(X_train, Y_train,i):
	...
	model.add(Dense(1))
	model.compile(loss='mse', optimizer='adam')
	history = model.fit(X_train, Y_train, epochs= 30, batch_size=1, validation_split=0.1, verbose=2, shuffle=True)
	#--------------------Loss图----------------------
	loss = history.history['loss']
	val_loss = history.history['val_loss']
	plt.figure(1, figsize=(14,8), dpi=600)
	plt.subplot(2, 2, i)
	plt.figure(1, figsize=(3.5, 2.5), dpi=600)
	# plt.plot(loss,'bo-', label='Training Loss',linewidth=2)
	# plt.plot(val_loss,'ro-', label='Validation Loss', linewidth=2)
	plt.plot(loss, 'bo-', linewidth=2)
	plt.plot(val_loss, 'ro-', linewidth=2)
	plt.title("IMF"+str(i), fontsize =15)
	plt.tight_layout()
	plt.tick_params(labelsize=15)
	plt.rc('font', family='Times New Roman')
	plt.savefig('C:/Users/Cy11/Desktop/隧道沉降/实验结果/fourth/')

	return model

def CNN_Model(X_train,Y_train):
	model = Sequential()
	model.add(Conv1D(16,2, padding="same",input_shape=(X_train.shape[1], X_train.shape[2])))
	...
	model.add(Dense(1))
	model.compile(loss='mse', optimizer='adam')
	model.fit(X_train, Y_train, epochs=300, batch_size=1, validation_split=0.1, verbose=2, shuffle=True)
	return model
...
def main():
	...
	print("shape=",data_regular_DL[0].shape)
	# model_CNN = CNN_Model(data_regular_DL[0],data_regular_DL[1])
	# y_predict=model_CNN.predict(data_regular_DL[2])
	# print(y_predict)
	...
	# #plt.plot(y_predict, 'red', label='CNN', linewidth=1)
	#
	...
	# #plt.plot(ceemdan_prediction, 'red', label='Proposed', linewidth=2.5, linestyle='--', marker='^',markersize=6)
