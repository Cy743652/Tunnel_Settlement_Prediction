...
def main():
    filename = 'data-570-1.csv'
    full_data, scaler = load_data('E:/Code/CEEMDAN-LSTM--master/csv/NB/'+filename)

    training_set_split = int(len(full_data) * (0.8))
    print(training_set_split)
    lookback_window = 2
    ...
    ###################################################################存入CSV
    a = pd.DataFrame(a[97:118],columns=["TRUE"])
    predict_svr = pd.DataFrame(predict_svr[97:118],columns=["predict_svr"])
    predict_bp = pd.DataFrame(predict_bp[97:118],columns=["predict_bp"])
    predict_elm = pd.DataFrame(predict_elm[97:118],columns=["predict_elm"])
    predict_lstm = pd.DataFrame(predict_LSTM[97:118],columns=["predict_lstm"])
    emd_prediction = pd.DataFrame(emd_prediction[97:118],columns=["predict_emd"])
    ceemdan_prediction = pd.DataFrame(ceemdan_prediction[97:118],columns=["predict_ceemdan"])
    all = pd.concat([a,predict_svr ,predict_bp,predict_elm,predict_lstm,emd_prediction,ceemdan_prediction],axis=1)
    all.to_csv('E:/Code/CEEMDAN-LSTM--master/csv/prediction/'+filename,index=False)

    ######################################################################
    d = pd.read_csv('E:/Code/CEEMDAN-LSTM--master/csv/prediction/'+filename, usecols=['TRUE', 'predict_svr', 'predict_bp',
                                                                                 'predict_elm', 'predict_lstm',
                                                                                 'predict_emd', 'predict_ceemdan'])

    plt.rc('font', family='Times New Roman')
    plt.figure(1, figsize=(15, 5))
    plt.plot(d.TRUE, 'black', linewidth=2.5, linestyle='--', marker='.')
    plt.plot(d.predict_svr, 'tan', linewidth=1)
    plt.plot(d.predict_bp, 'indianred', linewidth=1)
    plt.plot(d.predict_elm, 'khaki', linewidth=1)
    plt.plot(d.predict_lstm, 'lightsteelblue', linewidth=1)
    plt.plot(d.predict_emd, 'seagreen', linewidth=1)
    plt.plot(d.predict_ceemdan, 'red', linewidth=2.5, linestyle='--', marker='^', markersize=6)
    plt.grid(True, linestyle=':', color='gray', linewidth='0.5', axis='both')
    plt.xlabel('time(days)', fontsize=18)
    plt.ylabel('height(mm)', fontsize=18)
    # plt.title('551')
